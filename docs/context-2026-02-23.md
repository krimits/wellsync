...
3. **src/agents/morning_agent.py**: import + call `WellnessRetriever` (~15 γραμμές)
4. **Εκτέλεση migration**: `psql $DATABASE_URL -f migrations/add_knowledge_chunks.sql`
5. **Εκτέλεση ingestion**: `python -m src.knowledge.ingest` (one-time)
...
from src.knowledge.retriever import WellnessRetriever
